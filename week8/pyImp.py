...
def stepSim(t: int) -> None:
    global car_id#for unique car IDs
    #sim stats
    global queue_samples, sum_queue
    #1. Move along cars in links and push arrivals into downstream traffic-light queues.
    for u, v in links:
        pop_to_queue_if_arrived(u, v)#when car gets to dest move to downstream queue
    #2. Serve each intersection according to its traffic light condition.
    for node in nodes:
        serve_intersection(t, node)
    #3. Spawn boundary arrivals according to some random process.
    for u, v in links:
        if is_boundary_incoming_link(u, v):
            if random.random() < ARRIVAL_RATE:
                car = Car(car_id, t)
                car_id += 1
                # Try to enqueue the new car onto the link u->v
...
if __name__ == "__main__":
    ...
    print(f"Total cars completed: {completed}")
    print(f"Average travel time: {sum_tt / completed if completed > 0 else 0:.2f} ticks")
    print(f"Average queue length: {sum_queue / queue_samples if queue_samples > 0 else 0:.2f} cars")
    print(f"Total cars generated: {car_id}")
